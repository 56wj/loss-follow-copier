//+------------------------------------------------------------------+
//|                                             LossFollowCopier.mq5 |
//|  Opens a same-direction copy trade when a watched EA position     |
//|  reaches a floating loss threshold measured in points.            |
//+------------------------------------------------------------------+
#property strict
#property version   "1.00"
#property description "源单浮亏达到指定距离后，市价或挂单开同方向跟单。"
...
void CheckPositions()
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      ProcessSourceLevel(ticket, profile, 1);
      ProcessSourceLevel(ticket, profile, 2);
      ProcessSourceLevel(ticket, profile, 3);
   }
}
...
bool IsSourcePositionForProfile(const SourceProfile& profile)
{
   if(IsCopyPosition())
      return false;
   ...
   if(!IsAllowedSymbol(PositionGetString(POSITION_SYMBOL), profile.allowed_symbols))
      return false;

   if(profile.magic >= 0 && magic != profile.magic)
      return false;

   string comment = PositionGetString(POSITION_COMMENT);
   if(profile.comment_filter != "" && StringFind(comment, profile.comment_filter) < 0)
      return false;
                     ...
                     const double group_loss_price)
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      if(!PositionSelectByTicket(source_ticket))
         continue;
...
void ProcessSourceLevel(const ulong source_ticket, const SourceProfile& profile, const int level_index)
{
   ...
   // IsAlreadyCopied() scans other positions/orders, so re-select the source position
   // before reading its open price, volume, SL and TP.
   if(!PositionSelectByTicket(source_ticket))
      return;
   ...
   double source_open_price = PositionGetDouble(POSITION_PRICE_OPEN);
...
void CheckMinuteProfitClose()
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      SourceProfile profile;
      if(!MatchSourceProfile(PositionGetString(POSITION_SYMBOL),
...
bool SourceProfileByTicket(const ulong source_ticket, SourceProfile& profile)
{
   ...
   return MatchSourceProfile(PositionGetString(POSITION_SYMBOL),
                             PositionGetInteger(POSITION_MAGIC),
                             PositionGetString(POSITION_COMMENT),
                             profile);
}
...
bool DeleteCopyPending(const ulong order_ticket, const ulong source_ticket)
{
   ...
   request.magic = InpCopyMagic;
   request.comment = "LFC remove:" + IntegerToString((long)source_ticket);
...
bool CloseCopyPosition(const ulong copy_ticket, const ulong source_ticket)
{
   ...
   request.deviation = InpDeviationPoints;
   request.magic = InpCopyMagic;
   request.comment = "LFC close:" + IntegerToString((long)source_ticket);
   request.type_time = ORDER_TIME_GTC;
   request.type_filling = GetFillingType(symbol);
...
{
   return "LFC:" + IntegerToString((long)source_ticket) + ":L" + IntegerToString(level_index);
}
...
bool IsAlreadyCopied(const ulong source_ticket, const int level_index)
{
   ...
   string legacy_comment = "LFC:" + IntegerToString((long)source_ticket);
   int total = PositionsTotal();
   for(int i = total - 1; i >= 0; i--)
   {
      ulong ticket = PositionGetTicket(i);
