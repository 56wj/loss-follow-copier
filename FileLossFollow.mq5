...
string RoundTrackingKey(const SourceProfile& profile, const FileSourcePosition& source)
{
   if(profile.entry_mode == ENTRY_TOTAL_FLOATING_LOSS)
...
void StopActiveCopyRounds()
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      FileSourcePosition source;
      if(!FileSourceById(SourceTicketFromCurrentPosition(), source))
         continue;
...
bool CloseCopyPosition(const ulong copy_ticket, const ulong source_ticket)
{
   ...
   if(level_index > 0 && !FileSourceExists(source_ticket))
