...
enum ENUM_MEMORY_FOLLOW_ROLE
{
   MEMORY_FOLLOW_SENDER = 0,   // 源账号终端：发布持仓快照
   MEMORY_FOLLOW_RECEIVER = 1  // 跟单账号终端：读取快照并执行跟单
};
...
{
   if(g_winapi_mutex_handle == 0)
   {
      g_winapi_last_error = -104;
      return false;
   }

   uint wait_result = WaitForSingleObject(g_winapi_mutex_handle, timeout_ms);
   if(wait_result == WINAPI_WAIT_OBJECT_0 || wait_result == WINAPI_WAIT_ABANDONED)
      return true;

   g_winapi_last_error = -105;
   return false;
}

void WinApiMemoryUnlock()
{
   if(g_winapi_mutex_handle != 0)
...
bool WinApiMemoryOpen()
{
   ...
   g_winapi_view_address = MapViewOfFile(g_winapi_mapping_handle,
   ...
   g_winapi_mutex_handle = CreateMutexW(0, 0, mutex_name);
   if(g_winapi_mutex_handle == 0)
   {
      g_winapi_last_error = -103;
      WinApiMemoryClose();
      return false;
   }

   if(!WinApiMemoryLock(1000))
   {
      WinApiMemoryClose();
      return false;
   }

   WinApiMemoryHeader header;
   bool read_ok = WinApiReadHeader(header);
   bool valid = read_ok;
   if(read_ok && header.magic == 0)
   {
      header.magic = WINAPI_MEMORY_MAGIC;
      header.version = WINAPI_MEMORY_VERSION;
      header.sequence = 0;
      header.publish_tick_ms = 0;
      header.payload_size = 0;
      header.payload_crc32 = 0;
      header.capacity_bytes = g_memory_capacity_bytes;
      header.reserved = 0;
      valid = WinApiWriteHeader(header);
   }
   else if(read_ok)
   {
      valid = header.magic == WINAPI_MEMORY_MAGIC &&
              header.version == WINAPI_MEMORY_VERSION &&
              header.capacity_bytes == g_memory_capacity_bytes;
   }

   // If the sender restarts while the receiver still owns this named mapping,
   // continue the shared sequence instead of publishing a lower payload seq.
   if(valid && InpMemoryRole == MEMORY_FOLLOW_SENDER && header.sequence > 0)
      g_sender_sequence = (ulong)header.sequence;

   WinApiMemoryUnlock();
   if(!valid)
   {
      g_winapi_last_error = -106;
      WinApiMemoryClose();
      return false;
   }
   return true;
...
void WinApiMemoryClose()
{
   if(g_winapi_view_address != 0)
   {
      UnmapViewOfFile(g_winapi_view_address);
      g_winapi_view_address = 0;
   }
   if(g_winapi_mapping_handle != 0)
   {
      CloseHandle(g_winapi_mapping_handle);
      g_winapi_mapping_handle = 0;
   }
   if(g_winapi_mutex_handle != 0)
   {
...
int WinApiMemoryWrite(uchar &payload[], const int payload_size)
{
   if(g_winapi_view_address == 0 ||
      payload_size <= 0 ||
      payload_size > g_memory_capacity_bytes ||
      ArraySize(payload) < payload_size)
   {
      g_winapi_last_error = -107;
      return -1;
   }

   if(!WinApiMemoryLock(100))
      return -2;

   WinApiMemoryHeader header;
   bool valid = WinApiReadHeader(header) &&
                header.magic == WINAPI_MEMORY_MAGIC &&
                header.version == WINAPI_MEMORY_VERSION &&
                header.capacity_bytes == g_memory_capacity_bytes;
   if(!valid)
   ...
   if(header.sequence <= 0)
      header.sequence = 1;
   header.publish_tick_ms = (long)GetTickCount64();
   header.payload_size = payload_size;
   header.payload_crc32 = (long)WinApiMemoryCrc32(payload, payload_size);
   if(!WinApiWriteHeader(header))
   {
      WinApiMemoryUnlock();
      g_winapi_last_error = -110;
      return -5;
   }
...
int WinApiMemoryRead(uchar &payload[], const int payload_capacity)
{
   if(g_winapi_view_address == 0 || payload_capacity <= 0)
   {
      g_winapi_last_error = -111;
      return -1;
   }
   ...
   {
      WinApiMemoryUnlock();
      g_winapi_last_error = -112;
      return -3;
   }

   ulong sequence = (ulong)header.sequence;
   if(sequence == 0 || sequence == g_winapi_last_read_sequence)
   {
      WinApiMemoryUnlock();
      g_winapi_last_error = 0;
      return 0;
   ...
   int payload_size = (int)header.payload_size;
   if(payload_size <= 0 ||
      payload_size > payload_capacity ||
      payload_size > g_memory_capacity_bytes)
   {
      WinApiMemoryUnlock();
      g_winapi_last_error = -113;
      return -4;
   }
   ...
   {
      WinApiMemoryUnlock();
      g_winapi_last_error = -114;
      return -5;
   }

   uint actual_crc = WinApiMemoryCrc32(payload, payload_size);
   if(actual_crc != (uint)header.payload_crc32)
   {
      WinApiMemoryUnlock();
      g_winapi_last_error = -115;
      return -6;
   }

   g_winapi_last_read_sequence = sequence;
   WinApiMemoryUnlock();
   g_winapi_last_error = 0;
   return payload_size;
}
...
int OnInit()
{
   ...
   if(InpSharedMemoryCapacityKB < 64 || InpSharedMemoryCapacityKB > 16384)
   {
   ...
   if(InpMemoryRole == MEMORY_FOLLOW_SENDER)
      return InitMemorySender();
   return InitMemoryReceiver();
}

int InitMemorySender()
{
...
int InitMemoryReceiver()
{
   if(InpSourceStaleMilliseconds < 100)
   {
      Print("InpSourceStaleMilliseconds must be at least 100.");
      return INIT_PARAMETERS_INCORRECT;
   }
   ...
   // Global-variable names are limited to 63 characters.  Keep the scope
   // compact and hash account, channel, and copy magic instead of embedding
   // an unbounded user supplied channel name in every state key.
   g_prefix = "WMLFC_" +
              IntegerToString((long)WinApiMemoryNameHash(
                        ...
                        const MqlTradeResult& result)
{
   if(InpMemoryRole == MEMORY_FOLLOW_SENDER)
   {
      // Defer publication to OnTimer: position enumeration can be transient inside this callback.
      return;
   }
...
void SenderWriteSnapshot()
{
   ...
   string payload_text = SenderBuildSnapshotText(next_sequence, publish_tick_ms, now);
   SenderPublishPayload(payload_text, next_sequence, publish_tick_ms, false);
}

string SenderBuildSnapshotText(const ulong sequence,
                         const ulong publish_tick_ms,
                         const datetime snapshot_time)
{
   long login = AccountInfoInteger(ACCOUNT_LOGIN);
   string server = AccountInfoString(ACCOUNT_SERVER);
   string text = "META\tRLMC1\t" + EscapeMemoryField(InpChannelName) +
                 "\t" + IntegerToString(login) +
                 "\t" + EscapeMemoryField(server) +
                 "\t" + IntegerToString((long)snapshot_time) +
   ...
   text += "END\t" + IntegerToString(exported) +
           "\t" + IntegerToString((long)snapshot_time) +
           "\t" + IntegerToString((long)sequence) + "\n";
   return text;
}
...
void SenderWritePausedSnapshot(const string reason)
{
   ...
   string text = "META\tRLMC1\t" + EscapeMemoryField(InpChannelName) +
                 ...
                 "\t" + EscapeMemoryField(AccountInfoString(ACCOUNT_SERVER)) +
                 "\t0\t" + IntegerToString((long)next_sequence) +
                 "\t" + IntegerToString((long)publish_tick_ms) + "\n" +
                 "PAUSED\t" + EscapeMemoryField(reason) + "\n" +
                 "END\t0\t0\t" + IntegerToString((long)next_sequence) + "\n";
   SenderPublishPayload(text, next_sequence, publish_tick_ms, true);
}

bool SenderPublishPayload(const string text,
                    const ulong sequence,
                    const ulong publish_tick_ms,
                    const bool paused)
{
   uchar payload[];
   int payload_size = StringToCharArray(text, payload, 0, WHOLE_ARRAY, CP_UTF8);
   if(payload_size > 0 && payload[payload_size - 1] == 0)
      payload_size--;

   if(payload_size <= 0)
   {
      PrintSenderMemoryError("Encode memory snapshot failed", -1);
      return false;
   }

   if(payload_size > g_memory_capacity_bytes)
   {
      PrintSenderMemoryError("Memory snapshot exceeds configured capacity", payload_size);
      return false;
   }

   int written = WinApiMemoryWrite(payload, payload_size);
   if(written != payload_size)
   {
      int bridge_error = g_winapi_last_error != 0 ? g_winapi_last_error : written;
      PrintSenderMemoryError("Publish WinAPI shared memory snapshot failed", bridge_error);
      return false;
   }

   g_sender_sequence = sequence;
   if(InpPrintDebug)
   {
      datetime now = TimeLocal();
      if(now != g_last_debug_print)
      {
         g_last_debug_print = now;
         ulong elapsed_ms = GetTickCount64() - publish_tick_ms;
         PrintFormat("Memory snapshot published. seq=%I64u bytes=%d elapsed=%I64u ms paused=%s",
                     sequence,
                     payload_size,
                     elapsed_ms,
                     paused ? "true" : "false");
...
void PrintSenderMemoryError(const string action, const int error_code)
{
   datetime now = TimeLocal();
                          ...
                          const string local_symbol)
{
   ...
   volume = PartialRetryVolume(local_symbol, source_ticket, 1, volume);
      if(volume <= 0.0)
      {
...
bool IsSourcePositionForProfile(const SourceProfile& profile, const MemorySourcePosition& source)
{
...
void ProcessSourceLevel(const MemorySourcePosition& source, const SourceProfile& profile, const int level_index)
{
                                 ...
                                 const string action)
{
   ...
   PrintFormat("First memory copy entry blocked by Beijing time filter. action=%s source=%I64u level=L%d symbol=%s beijing=%s ranges=%s",
               action,
               source_ticket,
               level_index,
               symbol,
...
bool SourceProfileByTicket(const ulong source_ticket, SourceProfile& profile)
{
   ...
   if(!MemorySourceById(source_ticket, source))
      return false;
...
{
   return g_prefix + "TOTAL_" + state + "_";
}
...
bool HasLiveCopyOrPendingForSourceLevel(const ulong source_ticket, const int level_index)
{
   ...
   for(int i = position_total - 1; i >= 0; i--)
   {
      ulong copy_ticket = PositionGetTicket(i);
      if(copy_ticket == 0 || !PositionSelectByTicket(copy_ticket))
         continue;
...
void CleanupFinishedCopyState()
{
   ...
   for(int i = total - 1; i >= 0; i--)
   {
      ...
      string source_text = partial
                           ? StringSubstr(tail, StringLen("PARTIAL_"))
                           : tail;
                      ...
                      const int level_index)
{
   ...
   // MT5 requires RETURN filling for pending orders, independent of the
   // symbol's market-order filling flags.
...
bool CloseCopyPosition(const ulong copy_ticket, const ulong source_ticket)
{
   ...
   if(level_index > 0 && !MemorySourceExists(source_ticket))
...
{
   return "WMLFC:" + IntegerToString((long)source_ticket) + ":L" + IntegerToString(level_index);
}
...
{
   // A previous market request filled only part of the target volume.  Keep
   // the source eligible so the residual can be retried on the next scan.
...
bool LoadMemorySnapshot(const bool force)
{
   ulong now_tick_ms = GetTickCount64();
   ulong read_interval_ms = (ulong)(InpScanIntervalMs < 10 ? 10 : InpScanIntervalMs);
   if(!force &&
      g_last_memory_read_tick_ms > 0 &&
      now_tick_ms >= g_last_memory_read_tick_ms &&
      now_tick_ms - g_last_memory_read_tick_ms < read_interval_ms)
   {
      UpdateMemorySnapshotFreshness();
      return g_snapshot_fresh;
   }
   g_last_memory_read_tick_ms = now_tick_ms;

   if(g_winapi_view_address == 0 || ArraySize(g_memory_buffer) <= 0)
   {
      g_snapshot_fresh = false;
      return false;
   }

   int payload_size = WinApiMemoryRead(g_memory_buffer,
                                    ArraySize(g_memory_buffer));
   if(payload_size == 0)
   {
      UpdateMemorySnapshotFreshness();
      return g_snapshot_fresh;
   ...
   if(payload_size < 0)
   {
      PrintMemoryReadError("Read WinAPI shared memory snapshot failed",
                           g_winapi_last_error);
      UpdateMemorySnapshotFreshness();
      return g_snapshot_fresh;
   }

   string payload = CharArrayToString(g_memory_buffer, 0, payload_size, CP_UTF8);
   MemorySourcePosition loaded[];
   datetime snapshot_time = 0;
   ulong loaded_sequence = 0;
   ulong publish_tick_ms = 0;
   bool paused = false;
   if(!ParseMemorySnapshot(payload,
                           loaded,
                           snapshot_time,
                           loaded_sequence,
                           publish_tick_ms,
                           paused))
   {
      PrintMemoryReadError("Invalid WinAPI shared memory snapshot ignored", -1);
      UpdateMemorySnapshotFreshness();
      return g_snapshot_fresh;
   }
   ...
   if(InpPrintDebug)
   {
      ...
      if(now != g_last_snapshot_latency_log)
      {
         ...
         PrintFormat("Memory snapshot received. seq=%I64u positions=%d latency=%I64u ms",
                     loaded_sequence,
                     ArraySize(g_sources),
                     latency_ms);
      }
...
bool ParseMemorySnapshot(const string payload,
                         MemorySourcePosition &loaded[],
                         datetime &snapshot_time,
                         ulong &loaded_sequence,
                         ulong &publish_tick_ms,
                         bool &paused)
{
   ...
   string lines[];
   ushort line_separator = StringGetCharacter("\n", 0);
   ushort field_separator = StringGetCharacter("\t", 0);
   int line_count = StringSplit(payload, line_separator, lines);
   if(line_count <= 0)
      return false;
   ...
   for(int i = 0; i < line_count; i++)
   {
      ...
      string fields[];
      int field_count = StringSplit(line, field_separator, fields);
      if(field_count <= 0)
         continue;
      ...
      if(tag == "META")
      {
         if(field_count != 8 ||
            fields[1] != "RLMC1" ||
            UnescapeMemoryField(fields[2]) != InpChannelName)
            return false;

         snapshot_time = (datetime)StringToInteger(fields[5]);
         loaded_sequence = (ulong)StringToInteger(fields[6]);
         publish_tick_ms = (ulong)StringToInteger(fields[7]);
         have_meta = loaded_sequence > 0 && publish_tick_ms > 0;
         continue;
      }
      ...
      if(tag == "P")
      {
         ...
         source.open_price = StringToDouble(fields[8]);
         source.sl = StringToDouble(fields[9]);
         source.tp = StringToDouble(fields[10]);
         source.open_time = (datetime)StringToInteger(fields[11]);
      ...
      if(tag == "END")
      {
         if(field_count != 4)
            return false;
         expected_count = (int)StringToInteger(fields[1]);
         end_time = (datetime)StringToInteger(fields[2]);
         end_sequence = (ulong)StringToInteger(fields[3]);
         complete = true;
         break;
      }
   }
   ...
   if(!paused && snapshot_time <= 0)
      return false;
   return true;
}

string UnescapeMemoryField(string value)
{
...
void UpdateMemorySnapshotFreshness()
{
   if(!g_have_snapshot ||
      g_snapshot_time <= 0 ||
      g_snapshot_publish_tick_ms == 0)
   {
      g_snapshot_fresh = false;
      return;
   }
   ...
   g_snapshot_fresh = now_tick_ms >= g_snapshot_publish_tick_ms &&
                      now_tick_ms - g_snapshot_publish_tick_ms <=
                         (ulong)InpSourceStaleMilliseconds;
}

void PrintMemoryReadError(const string message, const int error_code)
{
   if(!InpPrintDebug)
      return;

   datetime now = TimeLocal();
   if(now == g_last_memory_error_log)
      return;
...
bool MemorySourceExists(const ulong source_id)
{
   MemorySourcePosition source;
   return MemorySourceById(source_id, source);
}
